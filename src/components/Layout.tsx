...
export default function Layout() {
  ...
  const getLicenseWarning = () => {
    if (tenantData?.id === 1 || tenantData?.name?.toLowerCase().includes('master') || tenantData?.name?.toLowerCase().includes('demo') || user?.is_superuser) {
      return null;
    }
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
          ...
          >
            {tenantData?.logo_url ? (
              <Box 
                ...
                sx={{ height: { xs: 30, sm: 45 }, width: 'auto', mb: 0.5 }} 
              />
            ) : (
            ...
            <Typography variant="caption" sx={{ fontWeight: 900, color: tenantData?.primary_color || 'primary.main', textTransform: 'uppercase', fontSize: { xs: '0.62rem', sm: '0.7rem' }, letterSpacing: '0.5px' }}>
            ...
            {location.pathname !== '/dashboard' && (
              <>
                <Button 
                  ...
                  onClick={() => navigate('/dashboard')}
                  sx={{ display: { xs: 'none', sm: 'inline-flex' }, fontWeight: 700, color: 'text.secondary', borderRadius: 2 }}
                >
                  Volver al Tablero
                </Button>
                <IconButton
                  onClick={() => navigate('/dashboard')}
                  sx={{ display: { xs: 'inline-flex', sm: 'none' }, color: 'text.secondary', p: 1 }}
      ...
      {user && (
        <Tooltip title="Síguenos en Instagram" arrow placement="left">
          <IconButton
            href="https://www.instagram.com/infagrasolution?igsh=ZTRkMmJ1bDJ6bDl0"
            target="_blank"
            rel="noopener noreferrer"
            sx={{
              position: 'fixed',
              ...
              color: 'white',
              boxShadow: '0 8px 30px rgba(220, 39, 67, 0.4)',
              width: { xs: 50, sm: 60 },
              height: { xs: 50, sm: 60 },
              zIndex: 9999,
              transition: 'all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
              '&:hover': {
                transform: 'scale(1.1) rotate(-10deg)',
                boxShadow: '0 12px 40px rgba(220, 39, 67, 0.6)',
              },
              animation: 'pulseInstagram 2.5s infinite',
              '@keyframes pulseInstagram': {
                '0%': { boxShadow: '0 0 0 0 rgba(220, 39, 67, 0.6)' },
                '70%': { boxShadow: '0 0 0 15px rgba(220, 39, 67, 0)' },
                '100%': { boxShadow: '0 0 0 0 rgba(220, 39, 67, 0)' }
              }
            }}
          >
            <svg 
